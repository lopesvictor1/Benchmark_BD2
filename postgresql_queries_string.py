...
class Database:
    def __init__(self):
        ...
        (sid SERIAL NOT NULL, pid INT NOT NULL, cost VARCHAR, PRIMARY KEY(sid, pid), \
        FOREIGN KEY(sid) REFERENCES Suppliers(sid), FOREIGN KEY(pid) REFERENCES Parts(pid))")             #linha que muda
        cur.execute(query)
    ...
    def insert_catalog(self, parts, suppliers):
        ...
        for i in range(parts):
            for j in range(suppliers):
                sid = j
                pid = i
                cost = str(random.randint(0, 1000))                                    #linha que muda
                row = [sid, pid, cost]
                data.append(row)
        args_str = ','.join(cur.mogrify("(%s,%s,%s)", x).decode("utf-8") for x in data)
        start = time.time() * 1000
    ...
    def update_catalog(self, parts, suppliers):
        ...
        for i in range(parts):
            pid = i
            cur.execute("UPDATE Catalog SET cost = '1' WHERE pid = " + str(pid) + " AND sid = " + str(sid) + ";")     #linha que muda
